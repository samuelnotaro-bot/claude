...
import { runBootRecovery } from "./autoRecovery.js";
...
try {
  ...
  if (!(await getLatestSynthesis())) {
    ...
    // until it completes. Chains straight into runBootRecovery so a fresh deploy
    ...
    backfillAll()
      .then(() => runBootRecovery())
      .catch((err) => app.log.error({ err }, "Backfill automatique au démarrage échoué"));
  } else {
    // Already has history: re-verify any recent zero-session rows possibly
    // poisoned by bugs fixed earlier today (see runBootRecovery), then drive
    // history automatically to "complete back to the retention floor, no
    // gaps" -- extends further back if needed, then closes any holes left by
    // cron runs missed while asleep (Render free-tier spin-down) or a
    // transient API failure. No-ops quickly if already complete. Also
    // available on demand via POST /api/data/deep-backfill and
    // /api/data/fill-gaps (see routes/api.ts), and re-run periodically while
    // awake (see scheduler.ts) so a large backlog keeps closing without
    // needing the app to restart.
    runBootRecovery().catch((err) => app.log.error({ err }, "Récupération automatique au démarrage échouée"));
  }
} catch (err) {
  app.log.error(err);
  process.exit(1);
